    ...
    O: PlatformAbstractions,
{
    ...
    config_value: usize,
    driver_state_machine: StateMachine,
    ...
    O: PlatformAbstractions + 'static,
{
    pub fn new_and_init(
        ...
        let in_endpoint = endpoints
           ...
           .unwrap()
           .clone();
        trace!("in_endpoint对应的端点: {:?}", in_endpoint);
        ...
        let out_endpoint = endpoints
           ...
           .unwrap()
           .clone();
        trace!("out_endpoint对应的端点: {:?}", out_endpoint);
        ...
        Arc::new(SpinNoIrq::new(Self {
            device_slot_id,
            in_endpoint: in_endpoint.doorbell_value_aka_dci(),
            out_endpoint: out_endpoint.doorbell_value_aka_dci(),
            config,
            interface_value,
            config_value,
            ...
            driver_state_machine: StateMachine::Waiting,
            ...
            CompleteCode::Event(TransferEventCompleteCode::Success) => {
                match self.driver_state_machine {
                    ...
                    StateMachine::Reading => {
                        trace!("-------------------------------------------------------------------------------------");
                        trace!("reading data");
                        let rec_data = self.read_data_buffer
                           .as_ref()
                           .unwrap()
                           .lock()
                           .as_mut()
                        ...
                        self.driver_state_machine = StateMachine::Waiting;
                    }
